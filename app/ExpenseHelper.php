...
class ExpenseHelper
{
	...
	{
		$this->product_id = 'DE785C4D';
		$this->api_url = 'https://envato.blarve.com/';
		$this->api_key = 'D76E1C74F9967CFB96A9';
	...
	{
		return true; // is_file($this->license_file);
	}
	...
	private function call_api($method, $url, $data = null)
	{
		...
		$this_ip = getenv('SERVER_ADDR') ?: (isset($_SERVER['SERVER_ADDR']) ? $_SERVER['SERVER_ADDR'] : getenv('REMOTE_ADDR')) ?:
	...
	public function verify_license($time_based_check = false, $license = false, $client = false)
	{
		...
		// 	$today = date('d-m-Y');
		// 	if (empty($_SESSION["f28eb66a037a20d"])) {
		// 		$_SESSION["f28eb66a037a20d"] = '00-00-0000';
		// 	}
		// 	if ($type == 1) {
		// 		$type_text = '1 day';
		// 	} elseif ($type == 3) {
		...
		// 		$type_text = $type . ' days';
		// 	}
		// 	if (strtotime($today) >= strtotime($_SESSION["f28eb66a037a20d"])) {
		// 		$get_data = $this->call_api(
		// 			'POST',
