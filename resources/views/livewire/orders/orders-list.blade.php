    ...
    <div class="card h-100 p-0">
        <div class="tw-py-1.5 tw-px-3 bg-base d-flex align-items-center flex-wrap gap-3 justify-content-between">
            <div class="d-flex align-items-center flex-wrap gap-3">
                <form class="navbar-search">
                    <input type="text" class="bg-base tw-px-3 tw-py-1.5 w-auto" name="search"
                        placeholder="{{ $lang->data['search_here'] ?? 'Search Here' }}" wire:model.live="search_query">
        ...
        <div class="d-flex flex-wrap gap-2 mt-3 mb-3">
            <div class="d-flex flex-wrap gap-2">
                ...
                <button
                    class="btn btn-sm rounded-pill {{ $quick_filter == 'todays_orders' ? 'btn-primary' : 'btn-outline-primary' }}"
                    wire:click="filterByQuick('todays_orders')">
                    Today's Orders
                </button>
                <button wire:click="filterByQuick('processing')"
                    class="btn btn-sm rounded-pill {{ $quick_filter == 'processing' ? 'btn-primary text-white' : 'btn-outline-primary' }}">
                    Processing Orders
                </button>

                <button wire:click="filterByQuick('ready')"
                    ...
                    <tbody>
                        @foreach ($orders as $item)
                            <tr class="tw-text-xs">
                                    ...
                                    <div class="tw-flex tw-flex-col">
                                        <div class="text-neutral-600">
                                            {{ $lang->data['order_id'] ?? 'Order ID' }} : <span
                                                class="tw-font-medium text-primary-light">{{ $item->order_number }}</span>
                                        </div>
                                        <div class="text-neutral-600">
                                            {{ $lang->data['order_date'] ?? 'Order Date' }} : <span
                                                class="tw-font-medium text-primary-light">{{ \Carbon\Carbon::parse($item->order_date)->format('d/m/y') }}</span>
                                        </div>
                                        <div class="text-neutral-600">
                                            {{ $lang->data['delivery_date'] ?? 'Delivery Date' }} : <span
                                                class="tw-font-medium text-primary-light">{{ \Carbon\Carbon::parse($item->delivery_date)->format('d/m/y') }}</span>
                                        </div>
                                        <div class="text-neutral-600">
                                            Total Items :
                                ...
                                <td class="">
                                    <p>{{ $item->customer_name ?? ($lang->data['walk_in_customer'] ?? 'Walk In Customer') }}
                                    </p>
                                    <p>{{ $item->phone_number ? getCountryCode() : '' }}{{ $item->phone_number ? (int) $item->phone_number : '' }}
                                    </p>
                                </td>
                                ...
                                <td class="">
                                    @if ($item->status == 0)
                                        <span
                                            class="badge  fw-semibold text-neutral-600 bg-neutral-200 px-20 py-9 radius-4 text-white">
                                            {{ $lang->data['pending'] ?? 'Pending' }}
                                        </span>
                                    @elseif($item->status == 1)
                                        <span
                                            class="badge  fw-semibold text-warning-600 bg-warning-100 px-20 py-9 radius-4 text-white">
                                            {{ $lang->data['processing'] ?? 'Processing' }}
                                        </span>
                                    @elseif($item->status == 2)
                                        <span
                                            class="badge  fw-semibold text-info-600 bg-info-100 px-20 py-9 radius-4 text-white">
                                            {{ $lang->data['ready_to_deliver'] ?? 'Ready To Deliver' }}
                                        </span>
                                    @elseif($item->status == 3)
                                        <span
                                            class="badge  fw-semibold text-success-600 bg-success-100 px-20 py-9 radius-4 text-white">
                                            {{ $lang->data['delivered'] ?? 'Delivered' }}
                                        </span>
                                    @elseif($item->status == 4)
                                        <span
                                            class="badge  fw-semibold text-danger-600 bg-danger-100 px-20 py-9 radius-4 text-white">
                                ...
                                <td>
                                    ...
                                    <div class="tw-flex tw-flex-col">
                                        <div class="text-neutral-600">
                                            {{ $lang->data['total_amount'] ?? 'Total Amount' }} : <span
                                                class="tw-font-medium text-primary-light">{{ getFormattedCurrency($item->total) }}</span>
                                        </div>
                                        <div class="text-neutral-600">
                                            @php
                                            ...
                                            {{ $lang->data['paid_amount'] ?? 'Paid Amount' }} : <span
                                                class="tw-font-medium text-primary-light">
                                                {{ getFormattedCurrency($current_paid_amount) }}</span>
                                            @if ($item->was_delivered_on_credit && $item->balance_amount > 0)
                                                ...
                                                @if ($item->credit_delivered_at)
                                                    <div class="text-warning fw-medium">
                                                        Credit Since :
                                                        {{ \Carbon\Carbon::parse($item->credit_delivered_at)->startOfDay()->diffInDays(today()->startOfDay()) }}
                                                        Days
                                                    </div>
                                                @endif
                                        ...
                                        @if ($paidamount < $item->total)
                                            @if ($item->status != 4)
                                                @can('payment_create')
                                                    <div class="tw-mt-1">
                                                        <button type="button"
                                                            class="btn rounded-pill btn-success-100 text-success-600 radius-8 tw-text-xs tw-py-1 tw-px-2 "
                                                            data-bs-toggle="modal" data-bs-target="#exampleModal"
                                                            wire:click="payment({{ $item->id }})">{{ $lang->data['add_payment'] ?? 'Add Payment' }}</button>
                                                    </div>
                                                @endcan
                                            @endif
                                        @else
                                            @if ($item->status != 4)
                                                <div class="tw-mt-1">
                                                    <button type="button"
                                                        class="btn rounded-pill btn-neutral-300 text-neutral-600 radius-8 tw-text-xs tw-py-1 tw-px-2 ">{{ $lang->data['fully_paid'] ?? 'Fully Paid' }}</button>
                                                </div>
                                            @endif
                                        @endif
                                ...
                                <td class="text-center">
                                    <div class="d-flex align-items-center gap-10 justify-content-center">
                                        ...
                                        @endcan
                                        <a href="{{ route('orders.print-all-tags', ['order' => $item->id]) }}"
                                            target="_blank"
                                        ...
                                        @can('order_print')
                                            <a href="{{ route('order.print', $item->id) }}" target="_blank"
                                                ...
                                                <iconify-icon icon="material-symbols-light:print-outline"
                        ...
                        <form action="#">
                            <div class="row">
                                        ...
                                        <ul>
                                            <li
                                                ...
                                                <span
                                                    ...
                                                    :</span>
                                                <span
                                                    class="text-secondary-light fw-medium">{{ $customer_name }}</span>
                                            </li>
                                ...
                                <div class="d-flex align-items-start justify-content-end gap-3 mt-24">
                                    ...
                                    <button type="button" wire:click.prevent="addPayment()"
