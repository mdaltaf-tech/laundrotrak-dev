        ...
        <div class="tw-py-1.5 tw-px-3 bg-base d-flex align-items-center flex-wrap gap-3 justify-content-between">
            ...
            @can('role_create')
                <button type="button"
                    ...
                    data-bs-toggle="modal" data-bs-target="#addyear" wire:click="resetFields()">
                ...
                <table class="table bordered-table sm-table mb-0">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" class="">
                                {{ $lang->data['total_permissions'] ?? 'Total Permissions' }}</th>
                            <th scope="col" class="">{{ $lang->data['users_with_role'] ?? 'Users With Role' }}
                            </th>
        ...
        <div class="modal-dialog modal-lg modal-dialog modal-dialog-centered">
            <div class="modal-content radius-16 bg-base">
                <div class="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                    <h1 class="modal-title text-md" id="exampleModalLabel">{{ $lang->data['add_role'] ?? 'Add Role' }}
                    </h1>
                    ...
                    <form action="#">
                        <div class="row">
                            <div class="col-12 mb-20">
                                <label for="name"
                                    class="form-label fw-semibold text-primary-light text-sm mb-8">{{ $lang->data['name'] ?? 'Name' }}
                                </label>
                                        ...
                                        <div class="d-flex align-items-center gap-10 fw-medium text-lg tw-col-span-1">
                                            <div class="form-check style-check d-flex align-items-center">
                                                @php
                                                    ...
                                                    if (!empty($currentRequiredItemsArray)) {
                                                        if ($currentRequiredItemsArray[0] == $permission['name']) {
                                                            $isMain = true;
                                                        }
                                                        if (
                                                            !in_array($permission['name'], $currentRequiredItemsArray)
                                                        ) {
                                                            if (
                                                                !isset(
                                                                    $selected_permissions[
                                                                        $currentRequiredItemsArray[0]
                                                                    ],
                                                                ) ||
                                                                (!$selected_permissions[
                                                                    $currentRequiredItemsArray[0]
                                                ...
                                                <input class="form-check-input radius-4 border border-neutral-500"
                                                    ...
                                                    wire:model.live="selected_permissions.{{ $permission['name'] }}"
                                                    @if ($isDisabled) disabled @endif
                                                    @if ($isMain) wire:change="handleCheckChange('{{ $key }}','{{ $permission['name'] }}',$event.target.checked)" @endif>
                                            </div>
                                            <label for="{{ $permission['name'] ?? '' }}"
                                                class="form-label fw-medium text-primary-light mb-0">
